...
import (
	...
	"github.com/bauerceptor/rendura/rendura_ebiten/internal/input"
	"math"
	"time"
	...
	piloop "github.com/bauerceptor/rendura/rendura_loop"
)
...
type EbitenGame struct {
	...
	// When true, indicates that the frame was rendered by rendura.Draw
...
func (g *EbitenGame) Update() error {
	...
	if g.ebitenFrame%(ebitenTPS/rendura.TPS()) == 0 {
		if !g.paused {
	...
	if g.ebitenFrame%(ebitenTPS/rendura.TPS()) == (ebitenTPS/rendura.TPS())-1 {
		if !g.paused {
			rendura.Update()
		...
		if time.Since(started).Seconds() > 1/float64(rendura.TPS()) {
			g.skipNextDraw = true // game is too slow. Try to keep up by discarding next rendura.Draw()
		}

		rendura.Time += 1.0 / float64(rendura.TPS())
		rendura.Frame++
	}
...
func (g *EbitenGame) Draw(screen *ebiten.Image) {
	if g.dirty { // draw only when needed to avoid CPU load on monitors >30 Hz
		...
		CopyCanvasToEbitenImage(rendura.Screen(), g.ebitenScreen)
