...
func TestShapes(t *testing.T) {
	t.Run("should draw shapes in the same way as in shapes.png", func(t *testing.T) {
		rendura.ResetPaletteMapping()
		rendura.ResetColorTables()
		rendura.Palette = rendura.DecodePalette(shapes)
		shapesSheet := rendura.DecodeCanvas(shapes)
		...
		for testName, shape := range tests {
			t.Run(testName, func(t *testing.T) {
				rendura.SetScreenSize(32, 32)
				rendura.Cls()
				rendura.SetColor(1)
				// when
				shape.draw()
				// then
				shapeArea := rendura.IntArea{X: shape.col * 32, Y: shape.row * 32, W: 32, H: 32}
				expected := shapesSheet.CloneArea(shapeArea)
				equal := rendura_test_helpers.AssertSurfaceEqual(t, expected, rendura.Screen())
				if !equal {
...
func line(x0, y0, x1, y1 int) func() {
	return func() {
		rendura.Line(x0, y0, x1, y1)
	}
}

func rect(x0, y0, x1, y1 int) func() {
	return func() {
		rendura.Rect(x0, y0, x1, y1)
	}
}

func rectfill(x0, y0, x1, y1 int) func() {
	return func() {
		rendura.RectFill(x0, y0, x1, y1)
	}
}

func circ(cx, cy, r int) func() {
	return func() {
		rendura.Circ(cx, cy, r)
	}
}

func circfill(cx, cy, r int) func() {
	return func() {
		rendura.CircFill(cx, cy, r)
	}
}
