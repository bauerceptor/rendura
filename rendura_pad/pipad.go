// Package rendura_pad provides functionality for a virtual gamepad
...
//     and on Switch it's "B". All of these map to the same constant rendura_pad.A,
...
//
// The rendura_pad package supports up to 16 buttons but defines only
...
// have even more than 16 buttons, but rendura does not support them.
package rendura_pad
...
func onButton(event EventButton, _ pievent.Handler) {
	...
	switch event.Type {
	case EventDown:
		buttonState[event.Player].SetDownFrame(event.Button, rendura.Frame)
	case EventUp:
		buttonState[event.Player].SetUpFrame(event.Button, rendura.Frame)
	}
}
