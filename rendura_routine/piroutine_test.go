package rendura_routine_test

import (
	"testing"

	"github.com/bauerceptor/rendura/rendura_loop"
	"github.com/bauerceptor/rendura/rendura_routine"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestPiroutine(t *testing.T) {
	r := rendura_routine.New(
		rendura_routine.Wait(2),
		rendura_routine.Printf("abc"),
		rendura_routine.Wait(5),
	)
	r.SetTracing(true)
	for r.Resume() {
	}
...
func TestCall(t *testing.T) {
	t.Run("should call callback", func(t *testing.T) {
		var executed = false
		step := rendura_routine.Call(func() {
			executed = true
		})
		// when
		result := step()
...
func TestSlowDown(t *testing.T) {
	t.Run("should wait n updates before running callback", func(t *testing.T) {
		executionCount := 0
		step := rendura_routine.SlowDown(2, func() bool {
			executionCount++
			return true
		})
		assert.False(t, step())
	...
	t.Run("should immediately run callback", func(t *testing.T) {
		executionCount := 0
		step := rendura_routine.SlowDown(0, func() bool {
			executionCount++
			return true
		})
		assert.True(t, step())
	...
	t.Run("should wait another n updates after callback returned false", func(t *testing.T) {
		executionCount := 0
		step := rendura_routine.SlowDown(3, func() bool {
			executionCount++
...
func TestRoutine_ScheduleOn(t *testing.T) {
	t.Run("should run callback on event", func(t *testing.T) {
		...
		routine := rendura_routine.New(step)
		// when
		handler := routine.ScheduleOn(rendura_loop.EventDraw)
		// then
		require.True(t, rendura_loop.Target().IsSubscribed(handler))
		rendura_loop.Target().Publish(rendura_loop.EventDraw) // runs callback and unsubscribes handlers
		assert.Equal(t, 1, executionCount)
		assert.False(t, rendura_loop.Target().IsSubscribed(handler))
	})
}
