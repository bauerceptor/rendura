...
func TestBuffer(t *testing.T) {
	t.Run("not found", func(t *testing.T) {
		buffer := rendura_ring.NewBuffer[int](4)
		assert.Equal(t, 0, buffer.Len())
		assert.Zero(t, *buffer.PointerTo(0))
	})

	t.Run("no cycle", func(t *testing.T) {
		buffer := rendura_ring.NewBuffer[int](3)
	...
	t.Run("cycle", func(t *testing.T) {
		buffer := rendura_ring.NewBuffer[int](2)
		*buffer.NextWritePointer() = 0
		*buffer.NextWritePointer() = 1
		*buffer.NextWritePointer() = 2
...
func assertHas[T any](t *testing.T, buffer *rendura_ring.Buffer[T], index int, value T) {
	t.Helper()
