...
package rendura_event

import (
	"log"
	"slices"

	"github.com/bauerceptor/rendura/internal/fileloc"
)
...
func (t *target[T]) Subscribe(event T, f func(T, Handler)) Handler {
	...
	h.event = event
	if !GlobalTracingOff {
		h.fileLoc = fileloc.Get("github.com/bauerceptor/rendura/rendura_event.") // skip pievent wrapper created in Subscribe function
	}
