...
func TestTarget_Publish(t *testing.T) {
	t.Run("should publish event to subscriber synchronously", func(t *testing.T) {
		target := rendura_event.NewTarget[string]()
...
func TestEventHandler_Unsubscribe(t *testing.T) {
	t.Run("should not unsubscribe other handler", func(t *testing.T) {
		target := rendura_event.NewTarget[string]()
		first := target.SubscribeAll(func(s string, _ rendura_event.Handler) {})
		target.Unsubscribe(first)
		eventReceived := false
		target.SubscribeAll(func(string, rendura_event.Handler) {
			eventReceived = true
		})
...
func TestTrackingTarget_Subscribe(t *testing.T) {
	t.Run("should return all subscription handlers", func(t *testing.T) {
		...
		target := rendura_event.NewTarget[string]()
		trackedTarget := rendura_event.Track(target)
		// when
		...
		assert.Equal(t, []rendura_event.Handler{handler1, handler2}, handlers)
		// and
...
func (s *Subscriber) Handler(e string, handler rendura_event.Handler) {
