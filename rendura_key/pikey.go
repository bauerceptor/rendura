// Package rendura_key provides functionality for a virtual keyboard.
//
...
//
// The rendura_key package supports up to 119 keys but defines only
...
// have even more than 119 keys, but rendura does not support them.
package rendura_key
...
func init() {
	onKey := func(event Event, _ pievent.Handler) {
		switch event.Type {
		case EventDown:
			keyState.SetDownFrame(event.Key, rendura.Frame)
		case EventUp:
			keyState.SetUpFrame(event.Key, rendura.Frame)
		}
	}
